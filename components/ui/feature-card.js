...
export function FeatureCard({ title, description, tech, image, link }) {
  return (
    <div>
      <div className="w-full relative rounded-lg overflow-hidden mx-auto bg-gradient-to-r from-[#1D2235] to-[#121318] p-8 border border-neutral-700">
        <Rays />
        <Beams />
        <div className="relative z-10">
          ...
          <motion.div className="py-4 relative z-10">
            ...
            <div className="rounded-lg ease-in duration-150 hover:rounded-full bg-[#584878] p-2"><LuGithub className="text-white font-bold w-4 h-4 hover:scale-125" /></div>
            </div>
            <p className="text-neutral-200 text-left  mt-1">{description}</p>
          </motion.div>
          <div className="flex items-center justify-start gap-2">

          {tech.map((techno, i) => (<span key={i} className="px-4 py-1.5 rounded-full relative backdrop-blur-2xl bg-[#584878] bg-opacity-50 text-white text-sm cursor-none">
              {techno}
          </span>))}
          </div>
